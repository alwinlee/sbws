<?php
    ...
    // 八關齋戒 3
    function syncLeaderPujaTB3($tbname,$clsname,$clsid,$area,$areaid,$findleaderinfo,$findvolunteerinfo,$classfullname,$mode){ // 班員 TB05, TB06
        $con = mysqli_connect("localhost","root","rinpoche");//connect_db("bwsouthdb");
        //從sdm_clsmembers找出該班學員出來
        $sql="select A.STU_ID,A.MembersSeq,A.TTL_ID from `sdmdb`.`sdm_clsmembers` A WHERE (A.CLS_ID='".$clsid."' AND A.TTL_ID <> '學員')";//echo $sql."<br>";
        $sql_result = mysqli_query($con, $sql);
    ...
    function syncMemberPujaTB3($tbname,$clsname,$clsid,$area,$areaid,$findleaderinfo,$findvolunteerinfo,$classfullname,$mode){ // 班員 TB05, TB06
        $con = mysqli_connect("localhost","root","rinpoche");//connect_db("bwsouthdb");
        //從sdm_clsmembers找出該班學員出來
        $sql="select A.STU_ID,A.MembersSeq,A.TTL_ID from `sdmdb`.`sdm_clsmembers` A WHERE (A.CLS_ID='".$clsid."' AND A.TTL_ID='學員')";//echo $sql."<br>";
        $sql_result = mysqli_query($con, $sql);
