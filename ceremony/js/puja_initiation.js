...
{
  $.ajax({
    async:false,url:"top.php",success: function (data) {$("#pageTop").append(data);}
  });
  ...
  $('#send').click(function (){
    ...
    for(i=1;i<=nMemberCnt;i++)
    {
      ...
      if (bPaid==true){//管理窗口
        ...
        sqlcmd="UPDATE `"+tbname+"` "+"SET `day2`="+joinday+",`traff2`=&#&#"+traffitem+"&#&#,";
        sqlcmd+="`cost2`="+fee+",`lock2`="+lock+",`pay2`="+paid+",`regdate2`=&#&#"+regnewdate+"&#&#,`paydate2`=&#&#"+paynewdate+"&#&#,";
        sqlcmd+="`paybyid2`=&#&#"+payernewid+"&#&#,`paybyname2`=&#&#"+payernewname+"&#&#,`memo2`=&#&#"+txtMemo+"&#&#";
        if(cancel>0){sqlcmd+=(",`cancel2`="+cancel+",`cancelinfo2`=&#&#"+cancelinfo+"&#&#");}
        else if(ocancel>0&&paid>0){sqlcmd+=(",`cancel2`=0");}//取消了又繳費
        sqlcmd+=clssqlcmd;
        sqlcmd+=" WHERE `idx`="+idx;
      }else{//幹部一般報名
        sqlcmd="UPDATE `"+tbname+"` "+"SET `day2`="+joinday+",`traff2`=&#&#"+traffitem+"&#&#,`cost2`="+fee+",`regdate2`=&#&#"+regnewdate+"&#&#,`memo2`=&#&#"+txtMemo+"&#&#";
        sqlcmd+=clssqlcmd;
        sqlcmd+=" WHERE `idx`="+idx;
      }
...
function showtable(data,classid,classname,area,region,classfullname)
{
  ...
  table1+="<thead><tr>";
  table1+="<th></th><th></th><th></th><th></th><th colspan='2'>"+item1+"</th>";
  table1+="<th></th>";
  table1+="<th></th><th></th>";
  ...
  table1+="<th style=\"width:120px;\">"+item2+"</th><th style=\"width:130px;\">"+item3+"</th>";//<th style=\"width:30px;\">訂餐</th>";
  table1+="<th style=\"width:120px;\">交通車次</th>";
  table1+="<th style=\"width:50px;\">車資</th><th style=\"width:120px;\">備註</th>";
  ...
  for(w=1;w<partsArray.length;w++)
  {
    ...
    //備註
    table1+="<td style='text-align:center;'><input style='width:110px' id='memo_"+(w)+"' serial='"+(w)+"' idx='"+idx+"' class='memo' type='text'"+" value='"+row[29]+"'></td>";
  ...
  $('.join').click(function(event) {
    ...
    if(item==1){$('#join2_'+serial).prop('checked',false);}
    if(item==2){$('#join1_'+serial).prop('checked',false);}
...
function getfee(index)
{
  ...
  //if ($('#join3_'+index).is(':checked')){if ($('#traffic3_'+index).val()!="Z"){fee+=unit;}}
