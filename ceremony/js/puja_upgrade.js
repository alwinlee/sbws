$(document).ready(function()
{
    ...
    $('#send').click(function(){
        ...
        for(i=1;i<=nMemberCnt;i++)
        {
            ...
            traff="Z";traffReal="Z";paid=0;joinday=0;meal=0;
            if ($('#join1_'+i).is(':checked')){joinday+=parseInt($('#place1_'+i).val());traff=$("#traffic1_"+i).val(); if(bPay){traffReal=traff;}}
            else{traff="Z";traffReal="Z";}
            if ($('#meal1_'+i).is(':checked')){meal+=1;}

            //if ($('#join3_'+i).is(':checked')){joinday+=100;traff="Z";traffReal="Z";}

            traff+=",";traffReal+=",";
            if ($('#join2_'+i).is(':checked')){joinday+=(parseInt($('#place2_'+i).val())*100);traff+=$("#traffic2_"+i).val(); if(bPay){traffReal+=$("#traffic2_"+i).val();}else{traffReal+="Z";}}
            else{traff+="Z";traffReal+="Z";}
            if ($('#meal2_'+i).is(':checked')){meal+=100;}
            ...
            if (bPaid==true){//管理窗口
                ...
                sqlcmd="UPDATE `"+tbname+"` "+"SET `day`="+joinday+",`meal`="+meal+",`traff`=&#&#"+traff+"&#&#,`traffReal`=&#&#"+traffReal+"&#&#,";
                ...
                sqlcmd="UPDATE `"+tbname+"` "+"SET `day`="+joinday+",`meal`="+meal+",`traff`=&#&#"+traff+"&#&#,`traffReal`=&#&#"+traffReal+"&#&#,`cost`="+fee+",`regdate`=&#&#"+regnewdate+"&#&#,`memo`=&#&#"+txtMemo+"&#&#";
                sqlcmd+=clssqlcmd;
                sqlcmd+=" WHERE `idx`="+idx;
            }
...
function showtable(data,classid,classname,area,region,classfullname){
    ...
    table1+="<th style=\"width:35px;\">參加</th><th style=\"width:30px;\">用餐</th><th style=\"width:90px;\">場次</th><th style=\"width:50px;\">車次</th>";//<th style=\"width:30px;\">訂餐</th>";
    table1 +="<th style=\"width:35px;\">參加</th><th style=\"width:30px;\">用餐</th><th style=\"width:90px;\">場次</th><th style=\"width:50px;\">車次</th>";//<th style=\"width:30px;\">訂餐</th>";
    ...
    for(w=1;w<partsArray.length;w++)
    {
		...
		table1+="<td style='text-align:center;'><input id='meal1_"+(w)+"' serial='"+(w)+"' idx='"+row[0]+"' class='meal1' type='checkbox'"+chkJoin+disabledlock+disableditem+"></td>";
        ...
        table1 +="<td style='text-align:center;'><select style='width:45px' id='traffic1_"+(w)+"' class='traffic' serial='"+(w)+"' idx='"+row[0]+"' name='traffic"+w+"' "+disableditem+disabledlock+">";
        for(kk=0;kk<trafftable1.length;kk+=2){sts=(traff==trafftable1[kk] ? "selected":" ");table1+="<option value='"+trafftable1[kk]+"' "+sts+">"+trafftable1[kk+1]+" - "+trafftable1[kk]+"</option>";}//{table1 +="<option value='' "+trafftable[kk]+"-"+trafftable[kk+1]+"></option>";}
        ...
        table1+="<td style='text-align:center;'><input id='meal2_"+(w)+"' serial='"+(w)+"' idx='"+row[0]+"' class='meal2' type='checkbox'"+chkJoin+disabledlock+disableditem+"></td>";
        ...
        traff=daytraff[1];if(traff==""||typeof(traff)=="undefined"){traff="Z";}

        disableditem=" disabled ";if (day2==2){disableditem=" ";}
        table1 +="<td style='text-align:center;'><select style='width:45px' id='traffic2_"+(w)+"' class='traffic' serial='"+(w)+"' idx='"+row[0]+"' name='traffic"+w+"' "+disableditem+disabledlock+">";
        for(kk=0;kk<trafftable2.length;kk+=2){sts=(traff==trafftable2[kk] ? "selected":" ");table1+="<option value='"+trafftable2[kk]+"' "+sts+">"+trafftable2[kk+1]+" - "+trafftable2[kk]+"</option>";}//{table1 +="<option value='' "+trafftable[kk]+"-"+trafftable[kk+1]+"></option>";}
        //for(kk=0;kk<trafftable2.length;kk+=2){selected=(traff==trafftable2[kk] ? "selected":" ");table1+="<option value='"+trafftable2[kk]+"' "+selected+">"+trafftable2[kk]+"</option>";}
        table1 +="</td>";
	...
	$('.join1').click(function(event){
		...
		var joincnt=0;
		var kaojoincnt=0;
		if ($('#join1_'+serial).is(':checked')){joincnt++;$('#traffic1_'+serial).prop('disabled',true);$('#place1_'+serial).prop('disabled',false);$('#meal1_'+serial).prop('disabled',false);}
		else{
		...
		//if ($('#join3_'+serial).is(':checked')){kaojoincnt++;}
		//if ($('#join4_'+serial).is(':checked')){kaojoincnt++;}

	    if (joincnt>=1){$('#notjoin_'+serial).prop('checked',false);$('#fee_'+serial).text(getfee(serial));}
		else if (kaojoincnt>=1){$('#fee_'+serial).text(getfee(serial));}
	    else{
			...
			$('#traffic2_'+serial+' option[value="Z"]').attr('selected', 'selected');//$("#traffic2_"+serial+" option")[0].selected=true;
	...
	$('.join2').click(function(event){
		...
		if ($('#join2_'+serial).is(':checked')){joincnt++;$('#traffic2_'+serial).prop('disabled',true);$('#place2_'+serial).prop('disabled',false);$('#meal2_'+serial).prop('disabled',false);}
		else{
			$('#meal2_'+serial).prop('checked',false);$('#meal2_'+serial).prop('disabled',true);
	...
	$('.notjoin').click(function(event){
		...
		$('#meal1_'+serial).prop('checked',false);$('#meal1_'+serial).prop('disabled',true);
        $('#traffic1_'+serial+' option[value="Z"]').attr('selected', selected); //$("#traffic1_"+serial+" option")[0].selected=true;
		//$('#traffic1_'+serial+' selected').val('Z');
		$('#traffic1_'+serial).val("Z").change();
		$('#traffic1_'+serial).prop('disabled',true);
		...
		$('#meal2_'+serial).prop('checked',false);$('#meal2_'+serial).prop('disabled',true);
		$('#traffic2_'+serial+' option[value="Z"]').attr('selected', 'selected');//$("#traffic2_"+serial+" option")[0].selected=true;
		//$('#traffic2_'+serial+' selected').val('Z');
		$('#traffic2_'+serial).val("Z").change();
        $('#traffic2_'+serial).prop('disabled',true);
...
function getfee(index){
	...
	{
	    if ($('#traffic1_'+index).val()!="Z"){fee+=roundfee;}
	}
	if ($('#join2_'+index).is(':checked'))
	{
	    if ($('#traffic2_'+index).val()!="Z"){fee+=roundfee;}
	}
