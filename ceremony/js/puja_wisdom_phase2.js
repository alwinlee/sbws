...
function showtable(data,classid,classname,area,region,classfullname){
    ...
    // row1
    table1+="<th></th><th></th><th></th><th></th><th colspan=\"3\">參加</th><th>車次</th><th>車資</th><th>特殊住宿需求</th><th>備註</th>";
    if(payitem=="YES"){table1+="<th>繳費</th>";}
    table1+="</tr><tr>";

    // row2
    table1+="<th style=\"width:30px;\">序</th><th style=\"width:85px;\">姓名</th><th style=\"width:65px;\">身份</th>";
    table1+="<th style=\"width:115px;\">梯次</th>";
    ...
    for(w=1;w<partsArray.length;w++) {
        ...
        table1 += `          <td style="width: 100px;border: 1px solid #bbb;text-align: center;padding: 4px 0;">
        ...
        table1+="<td style='text-align:center;'><input id='join3_"+(w)+"' item='3' serial='"+(w)+"' idx='"+row[0]+"' class='join' type='radio'"+chkJoin3+disableditem+JoinhiddenAll+"></td>";
    ...
    $('#queryresult').on('change', '.join', function (e) {
        ...
        if ($(this).val() == 0) {
            ...
            $(`#type1_${serial}, #type2_${serial}, #type3_${serial}, #type4_${serial}`).prop('disabled',false);

            if (!$('#type1_' + serial).is(':checked') && !$('#type2_'+serial).is(':checked') && !$('#type3_'+serial).is(':checked')) {
                $('#type1_' + serial).prop('checked', true);
            }
            $('#traffic1_' + serial).prop('disabled',false);
            $('#care_' + serial).prop('disabled',false);
        }
    });

    $('.join-old').click(function(event) {
        var serial=$(this).attr('serial');
        ...
        if (item==0){
            ...
            $('#type1_'+serial).prop('disabled',false);
            $('#type2_'+serial).prop('disabled',false);
            $('#type3_'+serial).prop('disabled',false);
            $('#type4_'+serial).prop('disabled',false);
            if (!$('#type1_'+serial).is(':checked') && !$('#type2_'+serial).is(':checked') && !$('#type3_'+serial).is(':checked')) {
                $('#type1_'+serial).prop('checked',true);
            }
            $('#traffic1_'+serial).prop('disabled',false);
...
function getfee(index) {
    ...
    if (fee<=0) {
        ...
        $('#notjoin_'+index).prop('disabled',false);
    } else {
