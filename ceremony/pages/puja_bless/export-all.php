<?php
  ...
  if($regioncode==""||$regioncode=="*"||$regioncode==" ") {
    if ($orderbydate=="YES"){
      $sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0) ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";
    } else {
      $sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0) ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";
    }

    $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
    $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
    $sql .= 'where (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0) ';
    ...
    if ($orderbydate=="YES"){
      $sql="SELECT * FROM ".$table_name." WHERE ((`day1`>0 OR `day2`>0  OR `day3`>0 OR `day4`>0) ".$areakey.") ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC,`idx` ASC";
    } else {
      $sql="SELECT * FROM ".$table_name." WHERE ((`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0) ".$areakey.") ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";
    }

    $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
    $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
    $sql .= 'where ((`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0) '.$areakey.')';
    $sql .= 'ORDER BY `CLS_ID` ASC, a.sex DESC,`STU_ID` ASC;';
  }
  ...
  $place_ary = array('-', '高雄學苑','華榮教室','文化教室','小港教室','仁武教室','河堤教室','屏東支苑','東港教室','萬丹教室','恆春教室');
  ...
  $idx++;$idx++;
  $item=$col[$idx]."3:".$col[$idx+2]."3";
  $objWorkSheet->mergeCells($item);
  $item=$col[$idx]."3";
  $objWorkSheet->setCellValue($item, $day4title);
  $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
  $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
  $item=$col[$idx]."4";
  $objWorkSheet->setCellValue($item,"參加");
  ...
  $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
  $item=$col[$idx+2]."4";
  $objWorkSheet->setCellValue($item,"連線場次");
  $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
  $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
  ...
  while($row=mysqli_fetch_array($result, MYSQLI_ASSOC)) {
    ...
    $day1 = 0;$day2=0;$day3=0;$day4=0;$day5=0;$fami1=0;$fami2=0;$fami3=0;$fami4=0;
    $day1 = $row["day1"];if($day1<=0){$traff1="";}
    $day2 = $row["day2"];if($day2<=0){$traff2="";}
    $day3 = $row["day3"];if($day3<=0){$traff3="";}
    $day4 = $row["day4"];
    $place4 = $row["service4"];
    $fami1=$row["family1"];$fami2=$row["family2"];$fami3=$row["family3"]; $fami4=$row["family4"];
    ...
    $objWorkSheet->setCellValue($col[$c].$iRow,$row["STU_ID"])//$row["STU_ID"])
          ...
          // ->setCellValue($col[++$c].$iRow,"")//$row["leaderinfo"])//$supportcalss)
           ...
           ->setCellValue($col[++$c].$iRow,$place4>0 ? $place_ary[$place4] : "")
