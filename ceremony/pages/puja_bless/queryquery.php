<?php
    ...
    while($row = mysqli_fetch_array($sql_result, MYSQLI_ASSOC)){//MYSQL_NUM))//MYSQL_ASSOC))
        ...
        // item4
        $data.=($row["lock4"]."|".$row["day4"]."|".$row["meal4"]."|".$row["family4"]."|".$row["service4"]."|");//51~55
        $data.=($row["joinmode4"]."|".$row["specialcase4"]."|".$row["traff4"]."|".$row["cost4"]."|".$row["pay4"]."|");//56~60
        $data.=($row["attend4"]."|".$row["regdate4"]."|".$row["paydate4"]."|".$row["payround4"]."|".$row["paybyid4"]."|");//61~65
        $data.=($row["paybyname4"]."|".$row["cancel4"]."|".$row["cancelinfo4"]."|".$row["memo4"]."|0|");//66~70

        // item5
        $data.=($row["lock5"]."|".$row["day5"]."|".$row["meal5"]."|".$row["family5"]."|".$row["service5"]."|");//51~55
        $data.=($row["joinmode5"]."|".$row["specialcase5"]."|".$row["traff5"]."|".$row["cost5"]."|".$row["pay5"]."|");//56~60
        $data.=($row["attend5"]."|".$row["regdate5"]."|".$row["paydate5"]."|".$row["payround5"]."|".$row["paybyid5"]."|");//61~65
        $data.=($row["paybyname5"]."|".$row["cancel5"]."|".$row["cancelinfo5"]."|".$row["memo5"]."|0|");//66~70
