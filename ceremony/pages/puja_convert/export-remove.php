<?php
    ...
    {
        if ($orderbydate=="YES"){$sql="SELECT * FROM ".$table_name." WHERE (`cancel1`> 0 OR `cancel2`>0) ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
        else{$sql="SELECT * FROM ".$table_name." WHERE (`cancel1`> 0 OR `cancel2`>0) ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
    }
    else
    {
        $Car=false;
        ...
        if ($orderbydate=="YES"){$sql="SELECT * FROM ".$table_name." WHERE ((`cancel1`> 0 OR `cancel2`>0) ".$areakey.") ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC,`idx` ASC";}
        else{$sql="SELECT * FROM ".$table_name." WHERE ((`cancel1`> 0 OR `cancel2`>0) ".$areakey.") ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
    }    
    ...
    $xlstitle=array("學員代號","姓名","母班班級","代碼","性別","年齡","區域","電話(宅)","電話(手機)","備註","收費窗口","取消窗口","","","","","");
    $xlstitleW=array(12,12,16,8,8,8,8,14,14,24,24,24,10,10,10,10,10,10,12,8);
    ...
    for($w=0;$w<count($xlstitle);$w++)
    {
     ...
     // 處理特殊欄位
    $objWorkSheet->mergeCells("M3:Q3");
    $objWorkSheet->setCellValue("M3","取消場次");
    $objWorkSheet->setCellValue("M4","A");
    $objWorkSheet->setCellValue("N4","B");
    $objWorkSheet->setCellValue("O4","C");
    $objWorkSheet->setCellValue("P4","D");    
    $objWorkSheet->setCellValue("Q4","E");     
    ...
    $objWorkSheet->getStyle("P4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("P4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);     
    $objWorkSheet->getStyle("Q4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("Q4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);     
     // end ---
    ...
    while($row=mysql_fetch_array($result, MYSQL_ASSOC))
    {
	  ...
	  { 
        ...
        $cancelinfo="";$cancelby="";
        $day=$row["cancel1"];
        $cancel=$row["cancelinfo1"];        
        $info1=explode("#",$cancel);if(count($info1)>=4){$cancelinfo.=$info1[3];}if(count($info1)>=7){$cancelby.=$info1[6];}
        $day2=($day==1?"1":"");
        $day4=($day==10?"1":"");
        $day5=($day==100?"1":"");
        $day=$row["cancel2"];
        $cancel=$row["cancelinfo2"];
        $info2=explode("#",$cancel);
        if(count($info2)>=4){if($cancelinfo!=""){$cancelinfo.=",";} $cancelinfo.=$info2[3];}
        if(count($info2)>=7){if($cancelby!=""){$cancelby.=",";} $cancelby.=$info2[6];}
        $day1=($day==1?"1":"");
        $day3=($day==10?"1":"");
        ...
        $objWorkSheet->setCellValue($col[$c].$iRow,$row["STU_ID"])				 
                     ...
                     ->setCellValue($col[++$c].$iRow,$cancelinfo)
                     ->setCellValue($col[++$c].$iRow,$cancelby)                     
                     ->setCellValue($col[++$c].$iRow,$day1)
                     ->setCellValue($col[++$c].$iRow,$day2)
                     ->setCellValue($col[++$c].$iRow,$day3)
                     ->setCellValue($col[++$c].$iRow,$day4)
                     ->setCellValue($col[++$c].$iRow,$day5);                    
    ...
    // 參加人數, 車資, 繳費 總計
    $sumitem=array($col[12],$col[13],$col[14],$col[15],$col[16]);
