<?php
    ...
    while($traffic_row = mysqli_fetch_array($pujatraffic_result, MYSQLI_ASSOC))
    {
        ...
        $traffinfo1[$i][1]=$traffic_row["traffname"]."[12/12(六)-上午場]";
        $traffinfo1[$i][2]=0;
        $traffinfo1[$i][3]="";
        $traffinfo1[$i][4]=0;

        $traffinfo2[$i][0]=$traffic_row["traffid"];
        $traffinfo2[$i][1]=$traffic_row["traffname"]."[12/12(六)-下午場]";;
        $traffinfo2[$i][2]=0;
        $traffinfo2[$i][3]="";
        $traffinfo2[$i][4]=0;

        $traffinfo3[$i][0]=$traffic_row["traffid"];
        $traffinfo3[$i][1]=$traffic_row["traffname"]."[12/13(日)-下午場]";;
        $traffinfo3[$i][2]=0;
        $traffinfo3[$i][3]="";
        $traffinfo3[$i][4]=0;
        $i++;
    ...
    // puja configure
    $sql         = "SELECT * FROM pujaconfig WHERE `php`='puja_convert.php'"; // 取得法會資料設定值
    ...
    if ($numrows > 0)
    {
        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC))
        {
            ...
            if ($row["day1"]<=0){continue;} // 沒報名或不參加
            if ($row["cost1"]<=0){continue;}
