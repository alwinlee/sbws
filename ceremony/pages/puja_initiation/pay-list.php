<?php
    ...
    $sql = "SELECT * FROM ".$table_name." WHERE `classname`='".$classname."' AND `pay2`>0 ORDER BY `idx` ASC";//'".$_POST["username"]."'";
    $con = mysqli_connect("localhost","root","rinpoche", "bwsouthdb");
    $result = mysqli_query($con, $sql);
    $numrows = mysqli_num_rows($result);

    $tablehtml_header="<tr align=\"center\"><td width=\"35px\" rowspan=\"2\">序</td><td width=\"120px\" rowspan=\"2\">姓名</td>";
    $tablehtml_header.="<td width=\"400px\" colspan=\"2\">搭車車次</td>";
    $tablehtml_header.="<td width=\"120px\" rowspan=\"2\">車資</td></tr>";
    $tablehtml_header.="<tr align=\"center\"><td width=\"200px\">11/23(六)-上午場</td><td width=\"200px\">11/23(六)-上午+下午+晚上場</td></tr>";
    ...
    if ($numrows > 0)
    {
        ...
        while($row=mysqli_fetch_array($result, MYSQLI_ASSOC))
        {
            ...
            if($row["day2"]<=0){continue;}
            $trafflist=explode(",",$row["traff2"]);
            if(count($trafflist)<3){continue;}
            if ($trafflist[3]=="Z"||$trafflist[3]== "ZZ"||$trafflist[3]==""){continue;}

            $day1="";$day2="";$day3="";
            if ($row["day2"]==1){$day1=$trafflist[2];}
            if ($row["day2"]==10){$day2=$trafflist[2];}
            //if ($row["day2"]==100){$day3=$trafflist[2];}

            $rowitem="<tr align=\"center\"><td>".$idx."</td><td>".$row["name"]."</td>";
            $rowitem.="<td>".$day1."</td><td>".$day2."</td><td>".$row["pay2"]."</td></tr>";
