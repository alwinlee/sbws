<?php
    ...
    {
        if ($orderbydate=="YES"){$sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `service1`>0) ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
        else{$sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `service1`>0) ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
        
        $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
        $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
        $sql .= 'where (`day1`>0 OR `day2`>0 OR `service1`>0) ';
        ...
        if ($orderbydate=="YES"){$sql="SELECT * FROM ".$table_name." WHERE ((`day1`>0 OR `day2`>0  OR `service1`>0) ".$areakey.") ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC,`idx` ASC";}
        else{$sql="SELECT * FROM ".$table_name." WHERE ((`day1`>0 OR `day2`>0 OR `service1`>0) ".$areakey.") ORDER BY `CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";}
        
        $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
        $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
        $sql .= 'where ((`day1`>0 OR `day2`>0 OR `service1`>0) '.$areakey.')';
        $sql .= 'ORDER BY `CLS_ID` ASC, a.sex DESC,`STU_ID` ASC;';
    }
    ...
    $xlstitleW=array(12,12,22,8,8,8,8,14,14,14,24,10,10,10,10,12,12,12,12,8);
    ...
    if($Car==true)
    {
        ...
        //$objWorkSheet->setCellValue($item,"Ｃ");
        //$objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        //$objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    }
    ...
    if($showtraffconfirmitem==true)
    {
        if ($day1trafficcnt>0)
        {
            ...
            $objWorkSheet->setCellValue($col[$begin].$top,"確認搭車-".$day1title);
            $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        ...
        if ($day2trafficcnt>0)
        {
            ...
            $item=$col[$begin].$top;
            $objWorkSheet->mergeCells($col[$begin].$top.":".$col[$end].$top);
            $objWorkSheet->setCellValue($col[$begin].$top,"-");
            $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    ...
    if($showtraffprepareitem==true)
    {
        ...
        if ($day1trafficcnt>0)
        {
            ...
            $item=$col[$begin].$top;
            $objWorkSheet->mergeCells($col[$begin].$top.":".$col[$end].$top);
            $objWorkSheet->setCellValue($col[$begin]."3","預計搭車-".$day1title);
            $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
            $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
            for($i=0,$w=0;$i<($day1trafficcnt*$roundcnt);$i+=$roundcnt,$w++)
            {
    ...
    while($row=mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
        ...
        $day1=0;$day2=0;$day3=0;$day4=0;$day5=0;$fami1=0;$fami2=0;
        $day1=$row["day1"];if($day1<=0){$traff1="";}
        $day2=$row["day2"];if($day2<=0){$traff2="";}
        $day3=$row["day3"];
        $fami1=$row["family1"];$fami2=$row["family2"];
        ...
        $traffitem1="";$traffitem2="";
        $traffitem1R="";$traffitem2R="";
        ...
        $regdate="";
        if($day1>0){$regdate=$row["regdate1"];}else if($day2>0){$regdate=$row["regdate2"];}else if($service1>0){$regdate=$row["regdate1"];}else{$regdate=$row["regdate3"];}
    ...
    // 參加人數, 車資, 繳費 總計
    $sumitem=array($col[9],$col[11],$col[12],$col[13],$col[14],$col[18],$col[20]);
