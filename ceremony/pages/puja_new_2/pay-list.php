<?php
    ...
    $sql="SELECT * FROM ".$table_name." WHERE `classname`='".$classname."' AND (`pay1`>0 OR `pay2`>0) ORDER BY `TTL_ID` ASC, `memberseq` ASC";//'".$_POST["username"]."'";
    $result=mysql_query($sql);
    $numrows=mysql_num_rows($result);

    $tablehtml_header="<tr align=\"center\"><td width=\"35px\" rowspan=\"2\">序</td><td width=\"80px\" rowspan=\"2\">姓名</td>";
    $tablehtml_header.="<td width=\"440px\" colspan=\"2\">搭車車次</td>";
    $tablehtml_header.="<td width=\"120px\" rowspan=\"2\">車資</td></tr>";
    $tablehtml_header.="<tr align=\"center\"><td width=\"220px\">2/19(一)(初四）</td><td width=\"220px\">2/20(二)(初五）</td></tr>";
    ...
    if ($numrows > 0)
    {
        ...
        {
            $idx++;
            if($row["day1"]<=0&&$row["day2"]<=0){continue;}
            ...
            $rowitem.="<td>".$day1."</td><td>".$day2."</td><td>".($row["pay1"]+$row["pay2"])."</td></tr>";
