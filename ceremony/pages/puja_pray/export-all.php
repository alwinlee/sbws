<?php
    ...
    {
        if ($orderbydate=="YES") {
            $sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0 OR `day5`>0 OR `day6`>0) ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC, `idx` ASC";
        } else {
            $sql="SELECT * FROM ".$table_name." WHERE (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0 OR `day5`>0 OR `day6`>0) ORDER BY `CLS_ID` ASC,`sex` DESC,`STU_ID` ASC";
        }
        $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
        $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
        $sql .= 'where (`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0 OR `day5`>0 OR `day6`>0) ';
        ...
        $areakey=" AND (".$sqlstring.")";
               $sql  = 'select a.*, b.PhoneNo_H as TEL, b.PhoneNo_C as CP ';
        $sql .= 'from `'.$table_name.'` as a  left join `studentinfo` as b on a.STU_ID=b.stuid ';
        $sql .= 'where ((`day1`>0 OR `day2`>0 OR `day3`>0 OR `day4`>0 OR `day5`>0 OR `day6`>0) '.$areakey.')';
        $sql .= 'ORDER BY `CLS_ID` ASC, a.sex DESC,`STU_ID` ASC;';
    }    
    ...
    $xlstitleW=array(12,12,22,8,8,8,8,14,14,24,6,6,10,10,6,6,10,10,6,6,10,10,6,6,10,10,6,6,10,10,6,6,10,10,8);
    ...
    for($z=0;$z<6;$z++)
    {
        ...
        $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $item=$col[$idx+1]."4";
        $objWorkSheet->setCellValue($item,"訂餐");
        $objWorkSheet->getStyle($item)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
        $objWorkSheet->getStyle($item)->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
        $item=$col[$idx+2]."4";
    ...
    $xlscartitlex=array($day1title."-去",$day1title."-回",$day2title."-去",$day2title."-回",$day3title."-去",$day3title."-回",$day4title."-去",$day4title."-回",$day5title."-去",$day5title."-回",$day6title."-去",$day6title."-回");
    ...
    while($row=mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
        ...
        if ($day4>0&&$traffReal4!=""&&$traffReal4!="Z"&&$traffCntReal4!=0){
            $carcnt++;
            for($x=0;$x<$day4trafficcnt;$x++){
                if($traffReal4==$traffinfo4[$x][0]){                    
                    if($traffCntReal4==1){$traffinfo4[$x][6]+=1;$traffinfo4[$x][6]+=$fami4;$car4go=$traffReal4."-".ceil(($traffinfo4[$x][5]+$traffinfo4[$x][6])/$CarVolume)."車";}
                    if($traffCntReal4==2){$traffinfo4[$x][7]+=1;$traffinfo4[$x][7]+=$fami4;$car4bk=$traffReal4."-".ceil(($traffinfo4[$x][5]+$traffinfo4[$x][7])/$CarVolume)."車";}
                    break;
                }
            }
        }        
        if ($day5>0&&$traffReal5!=""&&$traffReal5!="Z"&&$traffCntReal5!=0){
            $carcnt++;
            for($x=0;$x<$day5trafficcnt;$x++){
                if($traffReal5==$traffinfo5[$x][0]){                    
                    if($traffCntReal5==1){$traffinfo5[$x][6]+=1;$traffinfo5[$x][6]+=$fami5;$car5go=$traffReal5."-".ceil(($traffinfo5[$x][5]+$traffinfo5[$x][6])/$CarVolume)."車";}
                    if($traffCntReal5==2){$traffinfo5[$x][7]+=1;$traffinfo5[$x][7]+=$fami5;$car5bk=$traffReal5."-".ceil(($traffinfo5[$x][5]+$traffinfo5[$x][7])/$CarVolume)."車";}
                    break;
                }
            }
        }
        if ($day6>0&&$traffReal6!=""&&$traffReal6!="Z"&&$traffCntReal6!=0){
            $carcnt++;
            for($x=0;$x<$day6trafficcnt;$x++){
                if($traffReal6==$traffinfo6[$x][0]){                    
                    if($traffCntReal6==1){$traffinfo6[$x][6]+=1;$traffinfo6[$x][6]+=$fami6;$car6go=$traffReal6."-".ceil(($traffinfo6[$x][5]+$traffinfo6[$x][6])/$CarVolume)."車";}
                    if($traffCntReal6==2){$traffinfo6[$x][7]+=1;$traffinfo6[$x][7]+=$fami6;$car6bk=$traffReal6."-".ceil(($traffinfo6[$x][5]+$traffinfo6[$x][7])/$CarVolume)."車";}
                    break;
                }
            }
        }
        ...
        if($car4go!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car4go);}$c++;
        if($car4bk!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car4bk);}$c++; 
        if($car5go!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car5go);}$c++;
        if($car5bk!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car5bk);}$c++; 
        if($car6go!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car6go);}$c++;
        if($car6bk!=""){$objWorkSheet->setCellValue($col[$c].$iRow,$car6bk);}$c++;         
    }
    ///*/
    $iRow+=1;
    
    // SUM - 參加人數, 車資, 繳費 總計
    $sumitem=array($col[10],$col[11],$col[14],$col[15],$col[18],$col[19],$col[22],$col[23],$col[26],$col[27],$col[30],$col[31],$col[34],$col[36]);
