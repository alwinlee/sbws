<?php
    ...
    if ($Major=="YES"){$sql="select * from `bwsouthdb`.`".$tbname."` WHERE (`CLS_ID`='".$classid."' AND `titleid`>=0 AND `titleid`<5) ORDER BY Sex DESC, STU_ID ASC";
    }else{$sql="select * from `bwsouthdb`.`".$tbname."` WHERE (`CLS_ID`='".$classid."' AND `titleid`>=0 ) ORDER BY Sex DESC, STU_ID ASC";}
    ...
    while($row = mysqli_fetch_array($sql_result, MYSQLI_ASSOC)) { //MYSQL_NUM))//MYSQL_ASSOC))
        ...
        $data.=($row["family"]."|".$row["cost"]."|".$row["pay"]."|".$row["regdate"]."|".$row["paydate"]."|");
