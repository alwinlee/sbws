<?php
    ...
    for($w=0;$w<count($xlstitle);$w++)
    {
     ...
     // 處理特殊欄位
    $objWorkSheet->mergeCells("T3:Z3");
    $objWorkSheet->setCellValue("T3","參加梯次");
    ...
    $objWorkSheet->setCellValue("Z4","關懷員");

    $objWorkSheet->getStyle("T4:Z4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("T4:Z4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

    $objWorkSheet->mergeCells("K3:O3");
    $objWorkSheet->setCellValue("K3","特殊住宿需求安排");
    ...
    $objWorkSheet->setCellValue("K4","行動不便");
    $objWorkSheet->getStyle("K4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("K4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

    $objWorkSheet->setCellValue("L4","懷孕");
    $objWorkSheet->getStyle("L4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("L4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

    $objWorkSheet->setCellValue("M4","氣喘心臟");
    $objWorkSheet->getStyle("M4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("M4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

    $objWorkSheet->setCellValue("N4","打鼾");
    $objWorkSheet->getStyle("N4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("N4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

    $objWorkSheet->setCellValue("O4","其他症狀");
    $objWorkSheet->getStyle("O4")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
    $objWorkSheet->getStyle("O4")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
    // end ---
    ...
    while($row=mysqli_fetch_array($result, MYSQLI_ASSOC))
    {
	  ...
	  if ($day>=10)//有參加第二天
	  {
        ...
        $mainclass=$row["classfullname"];$curcls=$row["classfullname"];
        if ($row["titleid"]>=5){$mainclass=$row["otherinfo"];}
        $date=$dateCurr-date('Y',strtotime($row["age"]));

        $objWorkSheet->setCellValue($col[0].$iRow, "")
                     ->setCellValue($col[1].$iRow,$row["STU_ID"])
                     ...
                     ->setCellValue($col[9].$iRow,$row["PhoneNo_C"])
                     ->setCellValue($col[10].$iRow,$row["specialcase"]==1 ? "1":"")
                     ->setCellValue($col[11].$iRow,$row["specialcase"]==2 ? "1":"")
                     ->setCellValue($col[12].$iRow,$row["specialcase"]==3 ? "1":"")
                     ->setCellValue($col[13].$iRow,$row["specialcase"]==4 ? "1":"")
                     ->setCellValue($col[14].$iRow,$row["specialcase"]==5 ? "1":"")
                     ->setCellValue($col[15].$iRow,$row["memo"])
                     ->setCellValue($col[16].$iRow, "")
                     ->setCellValue($col[17].$iRow, "")
                     ->setCellValue($col[18].$iRow, "")
                     ->setCellValue($col[19].$iRow,$day==1 ? "1":"")
                     ->setCellValue($col[20].$iRow,$day==2 ? "1":"")
                     ->setCellValue($col[21].$iRow,$day==3 ? "1":"")
                     ->setCellValue($col[22].$iRow,$day==4 ? "1":"")

                     ->setCellValue($col[23].$iRow,$row["joinmode"]<=1 ? "1":"")
                     ->setCellValue($col[24].$iRow,$row["joinmode"]==2 ? "1":"")
                     ->setCellValue($col[25].$iRow,$row["joinmode"]==3 ? "1":"")
                     ->setCellValue($col[26].$iRow,$traff1)
                     ->setCellValue($col[27].$iRow,$cost>0 ? $cost:"")

                     ->setCellValue($col[28].$iRow,$row["TEL"])
                     ->setCellValue($col[29].$iRow," ".$row["CP"])
                     ->setCellValue($col[30].$iRow,$regdate)
                     ->setCellValue($col[31].$iRow,$pay)
                     ->setCellValue($col[32].$iRow,$paydate)
                     ->setCellValue($col[33].$iRow,$paybyname) ;
    }
