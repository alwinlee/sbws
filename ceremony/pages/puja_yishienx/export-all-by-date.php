...
*/
    ...
    // 考慮不同區域的報名窗口
    if($regioncode==""||$regioncode=="*"||$regioncode==" "){$sql="SELECT * FROM ".$table_name." WHERE (`day`> 0) ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC,`idx` ASC";}
    else
    {
        $areakey="";
        ...
        $sql="SELECT * FROM ".$table_name." WHERE (`day`> 0 ".$areakey.") ORDER BY `regdate` ASC,`CLS_ID` ASC,`TTL_ID` ASC,`memberseq` ASC,`idx` ASC";
    }
