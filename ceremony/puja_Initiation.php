...
<SCRIPT src="./js/puja_initiation.js?{BCB1FD17-B5F3-4340-A3D3-175FC1464387}" type="text/javascript"></SCRIPT>
  ...
  if ($numrows>0){
    ...
    if ($ceremAuth[4]>=1){
      while($row = mysqli_fetch_assoc($result_class)){
        ...
        if (strpos($row["Class"], $substringAdmin) !== false){ $yes = true; }
        if (!$yes) {continue;}
        $classname=$row["Class"]."-".$row["OrgName"];
        $regioncode = '1A';
        if ($row["ParentOrgId"] == "13" || $row["OrgId"] == "13"){
