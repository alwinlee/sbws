...
<SCRIPT src="./js/puja_convert.js?{E881ED85-2ECC-45C7-8B04-4E26BBE18D95}" type="text/javascript"></SCRIPT>
...
<?php
  ...
  if ($numrows>0){
    while($row = mysqli_fetch_assoc($result_class)){
      ...
      echo "<option value='".$row["CLS_ID"]."' regioncode='".$regioncode."'  AREAID='".$row["ARE_ID"]."' classname='".$row["Class"]."' >".$classname."</option>";
    }
    /*
    if ($ceremAuth[4]>=1){
      while($row = mysqli_fetch_assoc($result_class)){
        if (strpos($row["Class"], $substring1) !== false){ continue; }
        if (strpos($row["Class"], $substring2) !== false){ continue; }
        if (strpos($row["Class"], $substring3) !== false){ continue; }
        $classname=$row["Class"]."-".$row["OrgName"];
        $regioncode = '1A';
        if ($row["ParentOrgId"] == "13" || $row["OrgId"] == "13"){
          $regioncode = '3A';
...
/*
	if ($ceremAuth[4]>=1)
	{
		...
		echo "<td style='width:330px' align='right' >";
    echo "<input id='export' type='button' name='export' value='匯出'>&nbsp;&nbsp;&nbsp;&nbsp;";
		//echo "</td></tr>";
	}
*/
